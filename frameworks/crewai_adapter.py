...
class CrewAIAdapter(BaseFrameworkAdapter):
    ...
    def build_agent(self, system_prompt: str) -> RunnableAgent:
        # 1. Prepare Tools
        tools = [StructuredTool.from_function(func=fn, name=name, description=desc) for name, fn, desc in self.shim_tools]
...
class CrewAIRunnable(RunnableAgent):
    ...
    def run(self, task_str: str, context: Dict[str, Any] | None = None) -> Dict[str, Any]:
        try:
            task = Task(
