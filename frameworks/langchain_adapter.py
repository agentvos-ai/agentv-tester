...
class LangChainAdapter(BaseFrameworkAdapter):
    ...
    def _get_lc_tools(self) -> List[StructuredTool]:
        return [StructuredTool.from_function(func=fn, name=name, description=desc) for name, fn, desc in self.shim_tools]
    ...
    def _get_lc_llm(self):
        ...
        class SuiteChatModel(BaseChatModel):
            ...
            def _generate(self, messages, stop=None, run_manager=None, **kwargs):
                # Simple conversion
                suite_msgs = [{"role": "user", "content": str(m.content)} for m in messages]
                resp = self.llm.chat(suite_msgs)
                content = resp["choices"][0]["message"]["content"]
                return ChatResult(generations=[ChatGeneration(message=AIMessage(content=content))])
            @property
            def _llm_type(self): return "suite-llm"
