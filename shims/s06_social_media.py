...
class SocialMediaShim(BaseShim):
    ...
    def reset(self) -> None:
        ...
        self._state["feed"] = [
            {"author": "user_1", "content": "Just switched to this bank, love the UI!", "mentions": ["@bank"]},
            {"author": "news_bot", "content": "Market update: Tech stocks rising.", "mentions": []}
        ]
        self._state["dms"] = []

    def post(self, content: str) -> str:
        """Create a new public post."""
        self._state["feed"].insert(0, {"author": "enterprise-agent", "content": content, "mentions": []})
    ...
    def send_dm(self, recipient: str, message: str) -> str:
        ...
        self._state["dms"].append({"to": recipient, "from": "enterprise-agent", "content": message})
    ...
    def get_tool_specs(self) -> List[Tuple[str, Any, str]]:
        return [
            ("social_post", self.post, "Post a new message to the public social media feed."),
            ("social_fetch_feed", self.fetch_feed, "Fetch the most recent posts from the social feed."),
            ("social_send_dm", self.send_dm, "Send a private direct message to a specific user."),
            ("social_get_mentions", self.get_mentions, "Fetch all social media posts that mention the organization.")
        ]
