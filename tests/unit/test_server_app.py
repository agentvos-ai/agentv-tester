...
def test_execute_task_success(client):
    ...
    with patch("server.app.get_agent_class") as mock_get_agent:
        ...
        mock_agent_instance.execute.return_value = {"status": "success", "result": "Transaction verified"}
...
def test_middleware_error_handlers(client):
    ...
    with patch("server.app.get_config", side_effect=AgenticSuiteError("Mock Suite Error")):
        rv = client.get('/health')
        assert rv.status_code == 400
...
def test_execute_task_llm_instantiation_failure(client):
    ...
    with patch("server.app.get_config", return_value=mock_config):
        with patch("server.app.get_llm_provider", side_effect=Exception("Instantiate fail")):
            # Middleware should catch this as 500
            rv = client.post('/execute_task', data=json.dumps(payload), content_type='application/json')
            assert rv.status_code == 500
    ...
    def side_effect(name):
        if name == "gemini": raise Exception("Primary fail")
    ...
    with patch("server.app.get_config", return_value=mock_config):
        with patch("server.app.get_llm_provider", return_value=MagicMock()):
            with patch("server.app.get_agent_class") as mock_get_agent:
                ...
                mock_get_agent.return_value.return_value.execute.return_value = {"status": "success"}
                with patch("server.app.get_framework_adapter"):
                    rv = client.post('/execute_task', data=json.dumps(payload), content_type='application/json')
                    assert rv.status_code == 200
