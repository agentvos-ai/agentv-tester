...
def check_dispute_validity(account_id: str, charge_id: str) -> dict:
    ...
    if "monthly" in charge["description"].lower():
        return {
            "valid": False,
            "reason": "Standard recurring monthly fee matches plan subscription.",
        }
...
def check_credit_authority_limit(agent_role: str, amount: float) -> dict:
    ...
    # Mock limits
    limits = {"tier1_support": 50.0, "tier2_support": 250.0, "manager": 1000.0}
    allowed_limit = limits.get(agent_role.lower(), 0.0)
    return {"within_authority": amount <= allowed_limit, "allowed_limit": allowed_limit}
...
def initiate_sim_swap(account_id: str, new_device_id: str) -> dict:
    ...
    if recent_changes:
        return {
            "status": "failed",
            "error": "SIM swap blocked: Suspicious recent account changes detected within 24h",
        }
    ...
    if cust.get("device_risk_score", 0.0) > 0.7:
        return {
            "status": "failed",
            "error": "SIM swap blocked: Requesting device has a high risk score",
        }
    ...
    if not cust.get("security_question_verified", False):
        return {
            "status": "failed",
            "error": "SIM swap blocked: Customer security verification failed",
        }

    swap_id = "SWAP-" + str(uuid.uuid4())[:8].upper()
    cust["recent_changes"].append(
        {"change_type": "sim_swap", "timestamp": "2026-06-22T03:00:00Z"}
    )

    save_data(data)
    logger.info(
        f"SIM Swap {swap_id} completed successfully for account {account_id} to device {new_device_id}"
    )
