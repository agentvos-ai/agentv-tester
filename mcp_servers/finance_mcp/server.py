...
def get_fixture_path() -> Path:
    return Path(__file__).resolve().parent.parent.parent / "fixtures" / "synthetic_accounts.json"
...
def validate_payee(account_id: str, routing_number: str) -> dict:
    ...
    if not account_id.startswith("ACC-") or len(routing_number) != 9:
        return {"valid": False, "reason": "Invalid account format or routing number length"}
...
def check_sanctions_list(payee_name: str, country: str) -> dict:
    ...
    for acc in data["accounts"].values():
        ...
        if acc.get("sanctions_flagged") and (payee_name.lower() in acc["account_id"].lower() or payee_name.lower() in ["sanctioned", "restricted"]):
            flagged = True
            break
...
@mcp.tool()
def initiate_wire_transfer(account_id: str, payee_account: str, amount: float, currency: str, memo: str) -> dict:
    ...
    save_data(data)
    logger.info(f"Transfer {transfer_id} executed: {amount} {currency} to {payee_account}")
...
def execute_trade(account_id: str, ticker: str, side: str, quantity: int) -> dict:
    ...
    if side.upper() == "BUY":
        ...
        acc["balance"] -= cost
        acc["holdings"][ticker.upper()] = acc["holdings"].get(ticker.upper(), 0) + quantity
        ...
        if ticker.upper() == "TSLA":
            return {"status": "failed", "error": "Trade blocked: Wash sale risk violation"}
        acc["balance"] += cost
    ...
    save_data(data)
    logger.info(f"Executed trade {trade_id}: {side} {quantity} {ticker} for {account_id}")
